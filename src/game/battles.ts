import { JAVASCRIPT_AREA_ID, TYPESCRIPT_AREA_ID } from './areas'
import type { Battle, Enemy } from './types'

const enemy = (
  id: string,
  name: string,
  hp: number,
  attackName: string,
  attackDamage: number,
  glyph: string,
): Enemy => ({ id, name, hp, maxHp: hp, attackName, attackDamage, glyph })

export const battles: Battle[] = [
  {
    id: 1,
    areaId: JAVASCRIPT_AREA_ID,
    label: 'CHAPTER 01',
    title: 'Issue #101: Wrong Target',
    subtitle: 'ユーザー報告「攻撃対象がおかしい」。find()が返す“最初の一致”を追い、target selectorの再現条件を切り分ける',
    recommendedLevel: 1,
    expReward: 40,
    goldReward: 20,
    enemies: [
      enemy('slime-a', 'Slime', 34, 'Nibble', 6, '●'),
      enemy('goblin-a', 'Goblin', 72, 'Heavy Slash', 14, '▲'),
    ],
    skillIds: ['trace', 'pulse', 'nova'],
    unlockSkillId: 'viper',
  },
  {
    id: 2,
    areaId: JAVASCRIPT_AREA_ID,
    label: 'CHAPTER 02',
    title: 'QA Triage: Selector Drift',
    subtitle: '類似reportが複数skillで再現。filter()/&&/||で対象集合を洗い出し、配列順への暗黙依存がどこまで広がっているか調査する',
    recommendedLevel: 2,
    expReward: 80,
    goldReward: 30,
    enemies: [
      enemy('slime-b', 'Slime', 42, 'Nibble', 6, '●'),
      enemy('goblin-b', 'Goblin', 68, 'Heavy Slash', 15, '▲'),
      enemy('golem-b', 'Golem', 124, 'Stone Fist', 9, '■'),
    ],
    skillIds: ['trace', 'pulse', 'nova', 'viper', 'lock', 'alert'],
    unlockSkillId: 'moon-edge',
  },
  {
    id: 3,
    areaId: JAVASCRIPT_AREA_ID,
    label: 'FINAL CHAPTER',
    title: 'SEV-1: Targeting Outage',
    subtitle: 'deployでenemy配列の順序が変わりtargetingが不安定化。sort()/some()/reduce()で優先順位を明示し、本番serviceを復旧する',
    recommendedLevel: 3,
    expReward: 100,
    goldReward: 50,
    isBoss: true,
    enemies: [
      enemy('slime-c', 'Slime', 46, 'Bite', 3, '●'),
      enemy('goblin-c', 'Goblin', 84, 'Execution', 8, '▲'),
      enemy('boss-c', 'Boss', 156, 'Selector Cascade', 12, '◆'),
    ],
    skillIds: [
      'trace',
      'pulse',
      'nova',
      'viper',
      'lock',
      'alert',
      'moon-edge',
      'sweep',
      'judge',
    ],
    multiLineSkillIds: ['moon-edge', 'sweep', 'judge'],
  },
  {
    id: 4,
    areaId: TYPESCRIPT_AREA_ID,
    label: 'BATTLE 04',
    title: 'Typed Entry',
    subtitle: '型注釈を読み、値を調べる条件そのものは変わらないと見抜く',
    recommendedLevel: 1,
    expReward: 50,
    goldReward: 25,
    enemies: [
      enemy('slime-ts-a', 'Slime', 46, 'Type Ping', 4, '●'),
      enemy('goblin-ts-a', 'Goblin', 72, 'Strict Slash', 8, '▲'),
    ],
    skillIds: ['ts-scan', 'ts-guard', 'ts-label'],
    unlockSkillId: 'ts-union',
  },
  {
    id: 5,
    areaId: TYPESCRIPT_AREA_ID,
    label: 'BATTLE 05',
    title: 'Maybe Value',
    subtitle: 'unionとoptional propertyを追い、narrowing後の条件を読む',
    recommendedLevel: 2,
    expReward: 70,
    goldReward: 35,
    enemies: [
      enemy('slime-ts-b', 'Slime', 44, 'Undefined Bite', 4, '●'),
      enemy('goblin-ts-b', 'Goblin', 74, 'Union Slash', 8, '▲'),
      enemy('golem-ts-b', 'Golem', 104, 'Object Fist', 10, '■'),
    ],
    skillIds: ['ts-scan', 'ts-guard', 'ts-label', 'ts-union', 'ts-optional'],
    multiLineSkillIds: ['ts-union', 'ts-optional'],
    unlockSkillId: 'ts-narrow',
  },
  {
    id: 6,
    areaId: TYPESCRIPT_AREA_ID,
    label: 'BATTLE 06',
    title: 'Frontier Compiler',
    subtitle: '型の絞り込みとproperty keyを複合コードで追う',
    recommendedLevel: 3,
    expReward: 120,
    goldReward: 60,
    isBoss: true,
    enemies: [
      enemy('slime-ts-c', 'Slime', 46, 'Literal Spark', 3, '●'),
      enemy('goblin-ts-c', 'Goblin', 82, 'Narrow Edge', 7, '▲'),
      enemy('boss-ts-c', 'Boss', 132, 'Compiler Crash', 10, '◆'),
    ],
    skillIds: [
      'ts-scan',
      'ts-guard',
      'ts-label',
      'ts-union',
      'ts-optional',
      'ts-narrow',
      'ts-keyof',
    ],
    multiLineSkillIds: ['ts-union', 'ts-optional', 'ts-narrow', 'ts-keyof'],
  },
]
