...
export type BattleRouteArea = 'javascript' | 'typescript' | 'database'
...
const DATABASE_BATTLE_IDS = new Set([23])
...
function isDerivedUnlock(area: BattleRouteArea, battleId: number, progress: ProgressSlice) {
  if (area === 'database') {
    return battleId === 23 && isCleared(progress, 6)
  }
...
export function getBattleRouteLockReason(area: BattleRouteArea, battleId: number): string {
  if (area === 'database') {
    return 'Database ArchiveはTypeScript地方のFinal Bossを解決すると開く。まずShared Contractの異常を止めよう。'
  }
