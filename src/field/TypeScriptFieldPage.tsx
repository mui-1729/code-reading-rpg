...
export function TypeScriptFieldPage() {
    ...
    (interaction: FieldInteraction) => {
      if (interaction.kind === 'sign') {
        if ('learningHintId' in interaction) {
          const hint = typescriptLearningHintById[interaction.learningHintId]
          return hint
  ...
  const handleMove = useCallback(
    (direction: Direction) => {
      ...
      setFacing(direction)
      const next = movePlayer(typescriptField, position, direction)
  ...
  const handleInteract = useCallback(() => {
    ...
    const interaction = getInteractionInFront(typescriptField, position, facing)
  ...
  const cells = Array.from({ length: typescriptField.width * typescriptField.height }, (_, index) => ({
    ...
    y: Math.floor(index / typescriptField.width),
  }))
    ...
    <main className="app-shell field-shell title-screen">
      <section className="pixel-window field-panel">
        ...
        <div
          ...
          style={{ gridTemplateColumns: `repeat(${typescriptField.width}, minmax(0, 1fr))` }}
          aria-label="TypeScript Frontier field map"
        >
          {cells.map((cell) => {
            const blocked = typescriptField.blockedTiles.some((tile) => samePosition(tile, cell))
            const border =
              cell.x === 0 ||
              cell.y === 0 ||
              cell.x === typescriptField.width - 1 ||
              cell.y === typescriptField.height - 1
        ...
        {activeDialogue ? (
          ...
          >
            ...
            <pre className="learning-hint-code">
              <code>{activeLearningHint.codeLines.join('\n')}</code>
            </pre>
            <div className="learning-hint-notes">
              {activeLearningHint.notes.map((note) => (
                <p key={note}>• {note}</p>
              ))}
            </div>
        ...
        <div className="field-controls" aria-label="Field controls">
          ...
          <button
            ...
            onClick={() => navigate({ to: '/typescript' })}
          >
            STAGE SELECT
          </button>
        </div>
