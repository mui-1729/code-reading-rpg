...
test('clear済みOverworld Random Encounterは確認後に逃走し同じWorld位置/HPへ戻りrewardを得ない', async ({ page }) => {
  ...
  let commandBar = page.getByRole('group', { name: '戦闘コマンド' })
  ...
  let confirm = page.getByRole('group', { name: '逃走確認' })
  ...
  commandBar = page.getByRole('group', { name: '戦闘コマンド' })
  await expect(commandBar).toBeVisible()
  await commandBar.getByRole('button', { name: '逃げる' }).click()
  confirm = page.getByRole('group', { name: '逃走確認' })
