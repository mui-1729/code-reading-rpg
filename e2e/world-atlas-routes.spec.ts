...
test('ワールドマップは発見済みregionの接続関係をportal定義から表示する', async ({ page }) => {
  ...
  await expect(routes).toContainText('JavaScript草原')
  await expect(routes).toContainText('グリーンフィールド村')
  await expect(routes).toContainText('JavaScriptの森')
  await expect(routes).toContainText('JavaScript深層の森')
  await expect(routes).toContainText('TypeScript辺境')
