...
test('Random Encounterは戦う / アイテム / 逃げるの3commandから選ぶ', async ({ page }) => {
  ...
  const fightCommand = commands.getByRole('button', { name: '戦う', exact: true })
  const itemCommand = commands.getByRole('button', { name: 'アイテム', exact: true })
  await expect(fightCommand).toBeVisible()
  await expect(itemCommand).toBeVisible()
  await expect(commands.getByRole('button', { name: '逃げる', exact: true })).toBeVisible()
  await expect(fightCommand).toHaveAttribute('aria-pressed', 'false')
  await expect(itemCommand).toHaveAttribute('aria-pressed', 'false')
  await expect(page.getByRole('group', { name: 'スキル' })).toBeHidden()

  await fightCommand.click()
  await expect(fightCommand).toHaveAttribute('aria-pressed', 'true')
  ...
  await itemCommand.click()
  await expect(fightCommand).toHaveAttribute('aria-pressed', 'false')
  await expect(itemCommand).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByRole('group', { name: 'スキル' })).toBeHidden()
