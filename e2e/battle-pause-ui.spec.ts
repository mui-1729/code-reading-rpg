...
test('@cross-browser Battle equipment is read-only, while World equipment remains editable', async ({ page }) => {
  ...
  await expect(worldPause.locator('[data-equipment-slot="weapon"] header strong')).toHaveText('Guard Edge')
  await page.keyboard.press('Escape')
  ...
  await expect(battlePause.getByRole('button', { name: /アクセサリを選ぶ/ })).toBeDisabled()
  await expect(page.getByRole('dialog', { name: '武器を選ぶ' })).toHaveCount(0)
})
