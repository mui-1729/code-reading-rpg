...
test('reloadはBattle attemptをrollbackして開始HP / Itemへ戻す', async ({ page }) => {
  ...
  let opened = await openPatchKit(page)
  await opened.menu.getByRole('button', { name: /PATCH KIT ×1を使う/ }).click()
  await expect(page.locator('.player-panel .status-label-row strong')).toHaveText('64/108')

  opened = await openPatchKit(page)
  await expect(opened.detail).toContainText('PATCH KIT ×0')
  await expect(opened.menu.locator('.battle-item-state')).toContainText('この戦闘では使用済み')
  ...
  await expect(opened.menu.getByRole('button', { name: /PATCH KIT ×1を使う/ })).toBeEnabled()
...
test('browser backはBattle attemptをABORTしWorld snapshotを変更しない', async ({ page }) => {
  ...
  const opened = await openPatchKit(page)
  await opened.menu.getByRole('button', { name: /PATCH KIT ×1を使う/ }).click()
  await expect(page.locator('.player-panel .status-label-row strong')).toHaveText('64/108')
  await page.goBack()
