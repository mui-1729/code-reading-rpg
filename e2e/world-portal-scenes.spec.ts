...
test('Overworldの森入口は文字札ではなく木のarchとして見え、向いてActionすると森へ遷移する', async ({ page }) => {
...
test('村入口は木柵門、Village出口は草原へ抜ける門として別sceneになる', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '下へ移動' }).click()
  await expect(page.locator('.world-viewport')).toHaveAttribute('data-world-map', 'js-village')
...
test('Forestから第二集落を経由し、集落北口だけがDeep Forestのroot archになる', async ({ page }) => {
  ...
  await seedWorld(page, 'js-forest', { x: 2, y: 18 }, clearedThroughForest)

  const settlementGate = '.world-tile[data-world-x="1"][data-world-y="18"]'
