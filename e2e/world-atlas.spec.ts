...
test('進行条件付きregionはincident routeのgateをLOCKED表示しclear後はOPENになる', async ({ page }) => {
  await seedWorldAtlas(page)
  let atlas = await openAtlas(page)
  await expect(atlas.locator('[data-atlas-map="js-forest"]')).toContainText('LOCKED · CLEAR BATTLE 1')
