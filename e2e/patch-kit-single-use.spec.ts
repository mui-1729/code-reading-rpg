...
test('PATCH KITはItem browserから開き、在庫2個でも同一Battleで使用1回に制限する', async ({ page }) => {
  ...
  let { detail, menu, row } = await openPatchKit(page)
  await expect(row).toContainText('PATCH KIT')
  await expect(row).toContainText('×2')
  let patchKit = menu.locator('.patch-kit-action')
  ...
  ;({ detail, menu, row } = await openPatchKit(page))
  patchKit = menu.locator('.patch-kit-action')
  await expect(detail).toContainText('PATCH KIT ×1')
