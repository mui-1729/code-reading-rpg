...
Phase 2ではOverworldをField scaleへ移した。Phase 3ではpersistent safe checkpoint authorityと、Forest / Deep Forest間の第二有人拠点`js-forest-settlement`を追加した。Phase 4ではForestを55×41へ拡張し、**road terrainを置かず**、森・空き地・川・池・湿地・倒木横断・実物として見える痕跡で探索するLocal Mapへ再設計した。Phase 5ではDeep Forestを65×49へ拡張し、同じくroadを使わず、渓流・湿地・倒木横断・湧き水・巨大根を手掛かりにForestより長く濃い探索空間へ再設計した。
...
Forest 55×41 / Deep Forest 65×49のroadless geographyを現行runtime authorityとする。旧31×27 Deep Forestや旧「東→西main trail」をcurrent layout制約に戻さない。
...
Forestはroad networkを持たず、川・倒木・局所的な空き地・池・湿地・実際に描画された痕跡を手掛かりに上下左右へ探索するLocal Mapとして実装する。空き地同士を一本道で接続せず、Playerは地理と景観から進行方向を判断する。

Deep Forestもroad networkを持たない。東のForest Settlement側から入り、湿地の足跡、割れた実、羽根、渓流と倒木横断、根囲い、交差する巨大根、苔むした根、最深部の巨大根へと複数方向へ探索する。南側には部分回復の湧き水、main trace外にはTreasureを置き、単純な水平移動や`x <= N`だけで進行させない。詳細は`JAVASCRIPT_WORLD_TOPOLOGY.md`を参照する。
...
現行23×17 layoutは第二safe hubとして成立している。Forest / Deep Forestの通しplayを見て、生活空間・施設位置・拠点間距離は必要に応じて調整する。
...
ForestではBattle後7歩を安全区間にし、8歩目からRandom reviewを抽選する。Deep Forestはさらに広いためBattle後9歩を安全区間にし、10歩目からRandom reviewを抽選する。拡張面積をRandom Battle回数の水増しに使わない。
...
Forest 55×41 / Deep Forest 65×49 / Forest Settlementを含む現行topologyのAtlas統合は#373 / Phase 6で行う。Fog of WarはPhase 5で地理landmarkと任意探索が成立した後に導入する。
